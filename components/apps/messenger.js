...
componentDidMount() {
    ...
    this.setState({
        ...
        if (targetUsername) {
            ...
            // Clear the intent
            localStorage.removeItem(`${userId}_messenger_target_user`); // Remove correctly
            SessionManager.setItem('messenger_target_user', '');
        } else {
            // Default to first user if no target
            if (otherUsers.length > 0) this.selectUser(otherUsers[0]);
        }
    });
...
render() {
    ...
    loadMessages = () => {
        ...
        // Optimisation: Only update state if length changed (simple check)
        // ideally deep check but this is fine for now
        if (messages.length !== this.state.messages.length) {
            this.setState({ messages }, this.scrollToBottom);
        }
    }
